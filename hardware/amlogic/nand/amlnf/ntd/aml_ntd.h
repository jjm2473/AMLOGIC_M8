...
//	uint64_t offset;		    /* offset within the master NTD space */
//	uint32_t mask_flags;		/* master NTD flags to mask out for this partition */
//};
...
static inline uint32_t ntd_div_by_eb(uint64_t sz, struct ntd_info *ntd)
{
...
static inline uint32_t ntd_mod_by_eb(uint64_t sz, struct ntd_info *ntd)
{
	if (ntd->blocksize_shift)
...
static inline uint32_t ntd_div_by_ws(uint64_t sz, struct ntd_info *ntd)
{
...
static inline uint32_t ntd_mod_by_ws(uint64_t sz, struct ntd_info *ntd)
{
	if (ntd->pagesize_shift)
