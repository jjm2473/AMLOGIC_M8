...
***/
static int amlnand_oops_handle(struct amlnand_chip *aml_chip)
{
...
*Note          :
*****************************************************************************/
int amlnand_get_free_block(struct amlnand_chip *aml_chip)
{	struct hw_controller *controller = &aml_chip->controller;
	struct chip_operation *operation = & aml_chip->operation;
	struct chip_ops_para  *ops_para = &aml_chip->ops_para; 
	struct nand_flash *flash = &aml_chip->flash;
	...
	aml_nand_dbg(" total blk_addr = %d",total_blk);
...
int amlnand_save_info_by_name(struct amlnand_chip *aml_chip,unsigned char * info,unsigned char * buf,unsigned char * name,unsigned size)
{
	...
	unsigned int len, offset, offset_tmp, nand_boot, blk_addr,tmp_blk_addr,  amount_saved =0, pages_per_blk, arg_pages, pages_read;
	unsigned char phys_erase_shift, phys_page_shift;
	unsigned char  oob_buf[sizeof(struct _nand_arg_oobinfo)];
	unsigned short tmp_blk;
	int full_page_flag=0, ret = 0, i;
...
//get new block according to arg_type or update_flag
	if((arg_info->arg_valid)&&(!arg_info->update_flag) && (arg_info->arg_type == FULL_PAGE)){
		...
		if((arg_info->valid_page_addr + 2* arg_pages) > pages_read){
			blk_addr = amlnand_get_free_block(aml_chip);
					if( blk_addr < 0){
	...
	if(arg_info->arg_type == FULL_BLK){
		for(i=0; i<pages_read;){
				...
				while(amount_saved < size){
					...
					len = min(flash->pagesize, size - amount_saved);
					memset(aml_chip->user_page_buf, 0x0, flash->pagesize);		
...
int amlnand_configs_confirm(struct amlnand_chip *aml_chip)
{
	...
	for(i=0;tmp_num < device_num;tmp_num++,i++){
 	...
 	if(confirm_flag == 0){
		...
		ret = -1;
		aml_nand_msg("nand configs confirm failed and erase nand reset");
	}
...
int aml_nand_save_hynix_info(struct amlnand_chip *aml_chip)
{
	...
	unsigned char phys_erase_shift, phys_page_shift;	
	unsigned short  blk_addr,  tmp_blk,  nand_boot;
	...
	for(i=0; i<pages_read; i++){
		...
		for(j=0;j<save_cnt;j++) {
			memcpy((unsigned char *)(aml_chip->user_page_buf + MAX_CHIP_NUM*READ_RETRY_REG_NUM+j*READ_RETRY_REG_NUM+k*READ_RETRY_CNT),&retry_info->reg_offs_val_lp[k][j][0],READ_RETRY_REG_NUM);
		}
		#else
...
#ifdef AML_NAND_UBOOT		
	 //if flag = 2 then erase whole nand 
	 if(aml_chip->init_flag == NAND_BOOT_ERASE)
		 amlnand_oops_handle(aml_chip);
...
#else
		if(aml_chip->config_msg.arg_valid == 1){
			aml_chip->shipped_bbtinfo.valid_blk_addr = aml_chip->config_ptr->fbbt_blk_addr;	
			aml_nand_msg("nand shipped bbt at block %d",aml_chip->shipped_bbtinfo.valid_blk_addr);
			for(i=0; i<RESERVED_BLOCK_CNT; i++){
				if(aml_chip->reserved_blk[i] == 0xff){				
					aml_chip->reserved_blk[i] = aml_chip->config_ptr->fbbt_blk_addr;
					break;
