...
_mali_osk_errcode_t mali_pmu_reset(struct mali_pmu_core *pmu)
{
	...
	if (0 != cores_on_mask)
	{
		if (cores_on_mask & 0x1) {
			current_delay = cores_on_mask & 0xe?0xff:0xffff;
			mali_hw_core_register_write_relaxed(&pmu->hw_core, PMU_REG_ADDR_MGMT_SW_DELAY, current_delay);
			err = mali_pmu_send_command_internal(pmu, PMU_REG_ADDR_MGMT_POWER_UP, 0x1);
		}
		if (cores_on_mask & 0x2) {
			current_delay = cores_on_mask & 0xc?0xff:0xffff;
			mali_hw_core_register_write_relaxed(&pmu->hw_core, PMU_REG_ADDR_MGMT_SW_DELAY, current_delay);
			err = mali_pmu_send_command_internal(pmu, PMU_REG_ADDR_MGMT_POWER_UP, 0x2);
		}
		if (cores_on_mask & 0x4) {
			current_delay = cores_on_mask & 0x8?0xff:0xffff;
			mali_hw_core_register_write_relaxed(&pmu->hw_core, PMU_REG_ADDR_MGMT_SW_DELAY, current_delay);
			err = mali_pmu_send_command_internal(pmu, PMU_REG_ADDR_MGMT_POWER_UP, 0x4);
		}
		mali_hw_core_register_write_relaxed(&pmu->hw_core, PMU_REG_ADDR_MGMT_SW_DELAY, pmu->switch_delay);
		if (cores_on_mask & 0x8) {
			err = mali_pmu_send_command_internal(pmu, PMU_REG_ADDR_MGMT_POWER_UP, 0x8);
		}
		if (_MALI_OSK_ERR_OK != err) return err;
	}
...
_mali_osk_errcode_t mali_pmu_power_up(struct mali_pmu_core *pmu, u32 mask)
{
	...
	if (mask & 0x1) {
		current_delay = mask & 0xe?0xff:0xffff;
		mali_hw_core_register_write_relaxed(&pmu->hw_core, PMU_REG_ADDR_MGMT_SW_DELAY, current_delay);
		err = mali_pmu_send_command_internal(pmu, PMU_REG_ADDR_MGMT_POWER_UP, 0x1);
	}
	if (mask & 0x2) {
		current_delay = mask & 0xc?0xff:0xffff;
		mali_hw_core_register_write_relaxed(&pmu->hw_core, PMU_REG_ADDR_MGMT_SW_DELAY, current_delay);
		err = mali_pmu_send_command_internal(pmu, PMU_REG_ADDR_MGMT_POWER_UP, 0x2);
	}
	if (mask & 0x4) {
		current_delay = mask & 0x8?0xff:0xffff;
		mali_hw_core_register_write_relaxed(&pmu->hw_core, PMU_REG_ADDR_MGMT_SW_DELAY, current_delay);
		err = mali_pmu_send_command_internal(pmu, PMU_REG_ADDR_MGMT_POWER_UP, 0x4);
	}
	mali_hw_core_register_write_relaxed(&pmu->hw_core, PMU_REG_ADDR_MGMT_SW_DELAY, pmu->switch_delay);
...
_mali_osk_errcode_t mali_pmu_power_up_all(struct mali_pmu_core *pmu)
{
	...
	cores_on_mask = pmu->active_cores_mask;
	if (cores_on_mask & 0x1) {
		current_delay = cores_on_mask & 0xe?0xff:0xffff;
		mali_hw_core_register_write_relaxed(&pmu->hw_core, PMU_REG_ADDR_MGMT_SW_DELAY, current_delay);
		err = mali_pmu_send_command_internal(pmu, PMU_REG_ADDR_MGMT_POWER_UP, 0x1);
...
u32 mali_pmu_get_status(struct mali_pmu_core * pmu)
{
	...
	MALI_DEBUG_ASSERT_POINTER(pmu);
	mali_pmu_lock(pmu);
	ret = mali_hw_core_register_read(&pmu->hw_core, PMU_REG_ADDR_MGMT_STATUS);
	mali_pmu_unlock(pmu);
	return ret;
